...
//Calculates difference in time in minutes between start and end times
int changeMinutes(int startHours, int startMinutes, int endHours, int endMinutes)
{
    int change;
    ...
    {
        change = ((endHours - startHours) * 60) + (endMinutes - startMinutes);
    }
    ...
    {
        change = ((endHours - startHours - 1) * 60) + (60 - startMinutes + endMinutes);
    }
    ...
    {
        change = ((24 - startHours) + endHours) * 60 + (endMinutes - startMinutes);
    }
    else //If start hours are greater than end hours and start minutes are greater than or equal to end minutes
    {
        change = ((24 - startHours) + endHours - 1) * 60 + (60 - startMinutes + endMinutes);
    }
    return change;
}
...
int main()
{
    ...
    do
    {
        ...
        int change = changeMinutes(startHours, startMinutes, endHours, endMinutes); //Calculate the difference in time between the start time and end time
        ...
        cout << "Chnage in time: " << change << " minutes" << endl; //Prints elasped time
